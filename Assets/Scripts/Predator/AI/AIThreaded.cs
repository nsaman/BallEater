...
public class AIThreaded : MonoBehaviour
{
    ...
    void LookThroughFood()
    {
        ...
        float effectiveClosestEdibleFoodDistance = 99999999;
        GameObject closestSplitTarget = null;
        float effectiveClosestSplitTargetDistance = 99999999;
        ...
        Thread oThread = new Thread(LookForFoodThread);
        ...
        foreach (GameObject edible in edibles)
        {
            ...
            if (edibleRB != null && rb.mass > edibleRB.mass)
            {
                ...
                float currentFoodDistance = Vector3.Distance(edible.transform.position, thisTransform.position) / edibleRB.mass;
                if (currentFoodDistance < effectiveClosestEdibleFoodDistance)
                {
                    closestEdibleFood = edible;
                    effectiveClosestEdibleFoodDistance = currentFoodDistance;

                    if (globals.CANSPLIT &&
                        currentFoodDistance < effectiveClosestSplitTargetDistance &&
                        ...
                        maxShootRange > (posWithVelocity - edible.transform.position).magnitude)
                    {
                        ...
                        effectiveClosestSplitTargetDistance = currentFoodDistance;
                    }
                }
            }
        }
        ...
        foreach (GameObject food in foods)
        {
            ...
            {
                // check if it is the closest edible thing
                float currentFoodDistance = Vector3.Distance(food.transform.position, thisTransform.position) / foodRB.mass;
                if (currentFoodDistance < effectiveClosestEdibleFoodDistance)
                {
    ...
    static void LookForFoodThread() //List<GameObject> foods, List<GameObject> edibles)
    {
